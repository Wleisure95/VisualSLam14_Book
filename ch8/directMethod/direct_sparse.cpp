...
GradientThreadChoice GradientThread =Gradient_50;

#endif

#if Ret_gray
enum RetradiusChoice
{
  R_1 = 1,
  R_2 = 2
};
RetradiusChoice Retradius = R_2;
...
#if Ret_gray
	  float sumValue = 0.0;
	  for(int i = x-Retradius ; i<= x+Retradius ; ++i)
	    for (int j = y-Retradius; j <= y+Retradius ; ++j) 
	    {
	      sumValue += getPixelValue(i,j);
	    }
	sumValue /=( (2*Retradius +1)*(2*Retradius+1) );  
	//求得元素周围小块的平均灰度值
...
int main ( int argc, char** argv )
{
    ...
    for ( int index=0; index<10; index++ )
    {
	    ...
	    cout<<"key.size="<<keypoints.size()<<endl;
...
#if Ret_gray
		float grayscale = getPatchAverageGray(gray,kp.pt.x,kp.pt.y,Retradius);
#else
        ...
        for ( Measurement m:measurements )
        {
            ...
            cv::circle ( img_show, cv::Point2d ( pixel_prev ( 0,0 ), pixel_prev ( 1,0 ) ), 1, cv::Scalar ( b,g,r ), 2 );
            cv::circle ( img_show, cv::Point2d ( pixel_now ( 0,0 ), pixel_now ( 1,0 ) +color.rows ), 1, cv::Scalar ( b,g,r ), 5 );
