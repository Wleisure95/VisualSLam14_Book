...
void BuildProblem(const BALProblem* bal_problem, g2o::SparseOptimizer* optimizer, const BundleParams& params)
{
    ...
    {
        ConstVectorRef temVecPoint(raw_points + point_block_size * j, point_block_size);  //raw_points为所有points数组起点，从raw_points + point_block_size * j开始，连续point_block_size的数组。
...
void SolveProblem(const char* filename, const BundleParams& params)
{
    ...
    bal_problem.WriteToPCDFile("initial.pcd");
    ...
    bal_problem.WriteToPCDFile("final.pcd");
}
